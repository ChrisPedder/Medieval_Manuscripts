...
def download_clamm_data():
    ...
    print(clamm_data_directory)
    
    #check chosen directory exists, if not create it
    if not os.path.exists(clamm_data_directory):
        os.mkdir(clamm_data_directory)

    with open(clamm_data_directory + 'clamm_data', "wb") as handle:
...
def extract_clamm_data():
    ...
    root_path = Path.cwd()
    source_filename = str(root_path.parent.parent) + '/data/raw/clamm_data'
